...
function setupScene1Voice() {
    ...
    rec.onstart = () => { isRunning = true; };
    rec.onerror = () => { isRunning = false; try { rec.start(); } catch (e) { } };
    rec.onend = () => {
        isRunning = false;
        // Solo reinicia si la escena sigue activa
        if (game.state.current === 'scene1') {
